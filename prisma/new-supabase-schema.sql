-- Fresh Supabase Database Schema for Custom Backend
-- This is for a NEW Supabase project with clean database structure
...
ALTER TABLE exchange_symbol_info ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can manage their own risk settings" ON risk_settings
    FOR ALL USING (user_id = auth.uid());
...
-- RLS Policies for demo_balances
CREATE POLICY "Users can view their own demo balances" ON demo_balances
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Users can manage their own demo balances" ON demo_balances
    FOR ALL USING (user_id = auth.uid());
...
CREATE POLICY "Users can insert their own webhook events" ON webhook_events
    FOR INSERT WITH CHECK (user_id = auth.uid());

-- RLS Policies for exchange_symbol_info
CREATE POLICY "Users can view exchange symbol info" ON exchange_symbol_info
    FOR SELECT USING (true);

-- Insert default data for first user
-- This will be handled by the application when a user registers